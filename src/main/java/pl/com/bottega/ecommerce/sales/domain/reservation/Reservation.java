...
public class Reservation extends BaseAggregateRoot {
  ...
  /**
   * Sample function closured by policy </br> Higher order function closured by policy function</br>
   * </br> Function loads current prices, and prepares offer according to the current availability
   * and given discount
   *
  ...
  public Offer calculateOffer(DiscountPolicy discountPolicy) {
    ...
    for (ReservationItem item : items) {
      ...
      if (product.isAvailable()) {
        ...
        OfferItem offerItem =
            new OfferItem(product.generateSnapshot(), item.getQuantity(), discount);
